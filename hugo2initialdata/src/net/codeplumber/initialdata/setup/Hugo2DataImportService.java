 ...
 */
public abstract class Hugo2DataImportService {
    ...
    protected void importAllData(final AbstractSystemSetup systemSetup, final SystemSetupContext context,
        ...
        systemSetup.logInfo(context, String.format("Begin (hugo2) importing common data for [%s]", context.getExtensionName()));
        importCommonData(context.getExtensionName());

        systemSetup.logInfo(context,
                String.format("Begin (hugo2) importing product catalog data for [%s]", importData.getProductCatalogName()));
        ...
        for (final String contentCatalogName : importData.getContentCatalogNames()) {
            systemSetup.logInfo(context, String.format("Begin (hugo2) importing content catalog data for [%s]", contentCatalogName));
        ...
        if (syncCatalogs) {
            systemSetup
                    .logInfo(context, String.format("Synchronizing (hugo2) product catalog for [%s]", importData.getProductCatalogName()));
            final boolean productSyncSuccess = synchronizeProductCatalog(systemSetup, context, importData.getProductCatalogName(),
                    true);

            for (final String contentCatalogName : importData.getContentCatalogNames()) {
                systemSetup.logInfo(context, String.format("Synchronizing (hugo2) content catalog for [%s]", contentCatalogName));
            ...
            if (!productSyncSuccess) {
                ...
                systemSetup.logInfo(context,
                        String.format("Rerunning (hugo2) product catalog synchronization for [%s]", importData.getProductCatalogName()));
                if (!synchronizeProductCatalog(systemSetup, context, importData.getProductCatalogName(), true)) {
                    systemSetup.logInfo(context, String.format(
                            "Rerunning (hugo2) product catalog synchronization for [%s], failed. Please consult logs for more details.",
        ...
        for (final String storeName : importData.getStoreNames()) {
            systemSetup.logInfo(context, String.format("Begin (hugo2) importing store data for [%s]", storeName));
            importStore(context.getExtensionName(), storeName, importData.getProductCatalogName());

            systemSetup.logInfo(context, String.format("Begin (hugo2) importing job data for [%s]", storeName));
            importJobs(context.getExtensionName(), storeName);

            systemSetup.logInfo(context, String.format("Begin (hugo2) importing solr index data for [%s]", storeName));
            ...
            if (systemSetup.getBooleanSystemSetupParameter(context, ACTIVATE_SOLR_CRON_JOBS)) {
                systemSetup.logInfo(context, String.format("Activating (hugo2) solr index for [%s]", storeName));
    ...
    {
        systemSetup.logInfo(context, String.format("Begin (hugo2) synchronizing Product Catalog [%s]", catalogName));
        ...
        if (syncCatalogs)
        {
            ...
            {
                systemSetup.logInfo(context, String.format("Product (hugo2) Catalog [%s] sync has issues.", catalogName));
                return false;
            }
        }
